...
async def register(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    db_user = User(email=user.email, pw_hash=hash_password(user.password), role=user.role)
    db.add(db_user)
    await db.commit()
...
async def refresh(data: dict, db: AsyncSession = Depends(get_db)):
    ...
    access_token, access_jti = await create_access_token({"sub": user.email})
    refresh_token, refresh_jti = await create_refresh_token({"sub": user.email})
    await blacklist_token(payload["jti"], int((datetime.now(timezone.utc) + timedelta(days=7)).timestamp()))
    return {"access_token": access_token, "refresh_token": refresh_token, "token_type": "bearer"}

@router.post("/logout")
async def logout(token: str = Depends(oauth2_scheme)):
    payload = await verify_token(token, "access")
    await blacklist_token(payload["jti"], int((datetime.now(timezone.utc) + timedelta(minutes=15)).timestamp()))
